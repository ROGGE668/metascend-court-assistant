...
def create_app(dependencies: AppDependencies | None = None) -> FastAPI:
    ...
    @app.post("/ocr")
    async def ocr_extract(req: OcrRequest) -> dict:
        try:
            reader = LocalOCRReader()
            text = reader.extract_text(req.path)
            return {"path": req.path, "text": text}
        except Exception as e:
            logger.warning("OCR failed for %s: %s", req.path, e)
            return {"path": req.path, "text": "", "error": str(e)}
