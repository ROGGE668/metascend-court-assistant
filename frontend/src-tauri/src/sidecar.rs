...
impl SidecarManager {
    ...
    pub async fn set_env_overrides(&self, env: HashMap<String, String>) {
        let mut overrides = self.env_overrides.lock().await;
        overrides.clear();
        overrides.extend(env);
    }
    ...
    pub fn start(manager: Arc<Self>) -> impl std::future::Future<Output = Result<(), String>> + Send {
        ...
        let (pid, stdout, stderr) = Self::spawn_python(port, &manager.env_overrides).await?;
    ...
    async fn spawn_python(port: u16, env_overrides: &Mutex<HashMap<String, String>>) -> Result<(u32, ChildStdout, ChildStderr), String> {
        ...
        cmd.args(["run", "python", "-m", "src.api_server"])
            ...
            .kill_on_drop(false);
        {
            let overrides = env_overrides.lock().await;
...
fn resolve_uv_binary() -> Result<String, String> {
    ...
    // more robust.
    let candidates = ["uv", "/opt/homebrew/bin/uv", "/usr/local/bin/uv", "/usr/bin/uv", "/Users/hong/Library/Python/3.9/bin/uv", "/Users/hong/.local/bin/uv", "/Users/hong/.cargo/bin/uv"];
